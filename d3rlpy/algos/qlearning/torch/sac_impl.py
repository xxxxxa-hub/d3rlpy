...
class SACImpl(DDPGBaseImpl):
    ...
    def compute_actor_loss(self, states) -> torch.Tensor:
        dist = build_squashed_gaussian_distribution(
            self._modules.policy(states)
        )
    ...
    def update_temp(self, states) -> Dict[str, float]:
    ...
    def compute_target(self, next_states) -> torch.Tensor:
        with torch.no_grad():
            dist = build_squashed_gaussian_distribution(
                self._modules.policy(next_states)
            )
            action, log_prob = dist.sample_with_log_prob()
            entropy = self._modules.log_temp().exp() * log_prob
    ...
    def inner_update(self, states, actions, next_states, rewards, masks, grad_step: int) -> Dict[str, float]:
        ...
        metrics.update(super().inner_update(states, actions, next_states, rewards, masks, grad_step))
        return metrics
