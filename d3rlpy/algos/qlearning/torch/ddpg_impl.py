...
class DDPGBaseImpl(
    ...
    def update_critic(self, states, actions, next_states, rewards, masks) -> Dict[str, float]:
    ...
    def update_actor(self, states) -> Dict[str, float]:
        # Q function should be inference mode for stability
        self._modules.q_funcs.eval()

        self._modules.actor_optim.zero_grad()

        loss = self.compute_actor_loss(states)
    ...
    def inner_update(
        self, states, actions, next_states, rewards, masks, grad_step: int
    ) -> Dict[str, float]:
        metrics = {}
